...
static RULES: LazyLock<Vec<ClassificationRule>> = LazyLock::new(|| {
    let raw: &[(&str, FailureCategory)] = &[
        ...
        // Type errors
        (r"(?i)cannot find (value|function|type|module|struct|field|method)", FailureCategory::TypeError),
        ...
        (r"(?i)is not (a |an )?(function|defined|iterable)", FailureCategory::TypeError),
        (r"(?i)expected .+, found", FailureCategory::TypeError),
        (r"(?i)no method named", FailureCategory::TypeError),
        (r"(?i)no (associated |)item named", FailureCategory::TypeError),
        ...
        (r"(?i)unexpected end of (file|input)", FailureCategory::SyntaxError),
        ...
        (r"(?i)unused (import|variable|function)", FailureCategory::LintFail),
        ...
        (r"(?i)operation not permitted", FailureCategory::PermissionDenied),
...
mod tests {
    ...
    fn type_error_undefined() {
        assert_eq!(classify_failure("Cannot find value `foo`"), FailureCategory::TypeError);
    }

    #[test]
    fn type_error_mismatched() {
        assert_eq!(classify_failure("mismatched types: expected `i32`, found `String`"), FailureCategory::TypeError);
    }

    #[test]
    fn syntax_error() {
        assert_eq!(classify_failure("syntax error: unexpected token `}`"), FailureCategory::SyntaxError);
    }

    #[test]
    fn test_fail() {
        assert_eq!(classify_failure("test test_insert failed"), FailureCategory::TestFail);
    }

    #[test]
    fn test_assertion() {
        assert_eq!(classify_failure("assertion `left == right` failed"), FailureCategory::TestFail);
    }

    #[test]
    fn lint_fail() {
        assert_eq!(classify_failure("error[E0412]: cannot find type"), FailureCategory::LintFail);
    }

    #[test]
    fn build_fail() {
        assert_eq!(classify_failure("could not compile `my-crate`"), FailureCategory::BuildFail);
    }

    #[test]
    fn permission_denied() {
        assert_eq!(classify_failure("Permission denied (os error 13)"), FailureCategory::PermissionDenied);
    }

    #[test]
    fn timeout() {
        assert_eq!(classify_failure("request timed out after 30s"), FailureCategory::Timeout);
    }

    #[test]
    fn not_found() {
        assert_eq!(classify_failure("No such file or directory"), FailureCategory::NotFound);
    }

    #[test]
    fn runtime_panic() {
        assert_eq!(classify_failure("thread 'main' panicked at 'overflow'"), FailureCategory::RuntimeError);
    }

    #[test]
    fn unknown_for_gibberish() {
        assert_eq!(classify_failure("everything is fine"), FailureCategory::Unknown);
    }
