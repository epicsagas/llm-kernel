...
| `rustls-aws-lc-rs` | Proveedor TLS para todas las funciones basadas en reqwest (aws-lc-rs; la compilación cruzada requiere una cadena C) | ✅ |
| `rustls-ring` | Proveedor TLS `ring` (#93) — compilación cruzada sin cmake/nasm (basta un compilador C); llm-kernel instala el proveedor por defecto del proceso. Excluyente con `rustls-aws-lc-rs`; requiere `default-features = false` | |
...
| `dlp` | Prevención de pérdida de datos — escaneo determinista de contenido (secretos, PII coreana, rutas de archivos), política de datos por proveedor | |
| `dlp-fingerprint` | DLP L2 — coincidencia de huellas por coseno de documentos sensibles registrados sobre cualquier `EmbeddingProvider` | |
...
| `catalog-sync` | API de biblioteca de sincronización de catálogo — obtiene y fusiona models.dev en tiempo de ejecución (`provider::sync`), sin dependencias de CLI | |
| `catalog-sync-cli` | `catalog-sync` + dependencias de CLI (clap, anyhow) — binario `llm-kernel-sync-catalog` | |
...
```text
cargo run --bin llm-kernel-sync-catalog --features catalog-sync-cli -- --check   # mostrar drift
cargo run --bin llm-kernel-sync-catalog --features catalog-sync-cli              # escribir catalog.json
```
...
- **`dlp`** — prevención de pérdida de datos: `scan()` detecta secretos/PII coreana/rutas con spans de bytes y grado de `Sensitivity`; `DataPolicy` en `ServiceDescriptor` + `policy::lookup` resuelve Allow/Redact/Warn/ReRoute/Block; `dlp-fingerprint` añade coincidencia por coseno; `ContentClassifier` es la interfaz opcional para modelos locales
...
| dlp | precisión/recall/F1 por categoría, exactitud de sensibilidad, tasa de falsos positivos en corpus benigno (solo eval-full) |
