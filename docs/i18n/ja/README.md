...
`discovery-async` 機能はプラグ可能な `DiscoverySource` トレイトを公開し、モデル一覧を単一のインターフェース背後にある任意の非同期バックエンドから取得できます：
...
`FederatedSearch` は複数の `AsyncVectorIndex` バックエンド（Qdrant、Elasticsearch、…）を並行照会し、バックエンドごとのタイムアウトを適用して遅いリモート1つがクエリを停滞させないようにし、生存者をマージします。デフォルト戦略は **RRF** です。ランクベースでスケール不変であるため、異種の生スコア（Qdrantのコサイン、Elasticsearchの `_score`、TurboVecの生コサイン）が正規化なしで正しく融合します。`federation` 機能の背後にあります（依存関係に `features = ["federation"]` を追加）。
...
同期型 `TurbovecIndex` は純粋な `federate_results` マージ経由で参加します — 直接検索し、そのリストを非同期バックエンドと並べて折り込みます。
...
`VectorIndex` トレイトは llm-kernel で定義されます（依存関係なし）。TurboQuant 圧縮（最大16倍、SIMD検索）による具体的な実装は [`llm-kernel-vector-index`](https://github.com/epicsagas/llm-kernel-vector-index) を参照。
...
`PromptTemplate` は `{{variable}}` プレースホルダを置換し、few-shot 例を本文の前に描画します。設定駆動プロンプトのために `Serialize`/`Deserialize` を導出します。
