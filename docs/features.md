...
| `rustls-ring` | ring TLS provider (#93) — cross-compiles without cmake/nasm (a plain C compiler still suffices); llm-kernel installs the process-default provider at runtime. Mutually exclusive with `rustls-aws-lc-rs`; requires `default-features = false` |
